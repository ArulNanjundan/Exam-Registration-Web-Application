<?php
    ...
    if (!$connection) {
        die("<script>alert('Database connection failed'); window.location.href='user.html';</script>");
    }
    ...
    if (!($search === '') && $email !== '') {
        ...
        $email_check_sql = "SELECT * FROM registration WHERE Application_no = ? AND emaill = ?";
        $email_check_stmt = mysqli_prepare($connection, $email_check_sql);
        mysqli_stmt_bind_param($email_check_stmt, "ss", $raw_number, $email);
        mysqli_stmt_execute($email_check_stmt);
        mysqli_stmt_store_result($email_check_stmt);

        if (mysqli_stmt_num_rows($email_check_stmt) == 0) {
            echo "<script>alert('Unauthorized Email. Only open your application.');window.location.href='search_user.html';</script>";
            exit();
        }
        ...
        if (!$stmt) {
            die("<script>alert('Database error'); window.location.href='user.html';</script>");
        }
        ...
        if (mysqli_stmt_num_rows($stmt) > 0) {
            ...
            echo "<script>alert('Not Available the Application Number');window.location.href='search_user.html';</script>";
        }
    } 
    else {
        echo "<script>window.location.href='search_user.html';</script>";
    }
